...
export default function App() {
  ...
  const loadTagLibrary = useCallback(async () => {
    ...
    if (data) {
      const lib = {};
      data.forEach(({ tag, dimension }) => {
        if (!lib[dimension]) lib[dimension] = [];
        lib[dimension].push(tag);
      });
      setTagLibrary(lib);
    }
  }, []);
  ...
  useEffect(() => { loadProducts(); loadTagLibrary(); }, [loadProducts, loadTagLibrary]);
  ...
  const generatePDF = async () => {
    ...
    try {
      const res = await fetch(`${CATALOGUE_URL}/generate-catalogue`, {
        ...
        body: JSON.stringify({
          products: sel.map(p => ({
            name: p.name, origin: p.category || "India", category: p.category || "General",
            price: Math.round(p._price), description: p.description || "",
  ...
  const openTagReview = async (product) => {
    ...
    setTagSuggestions({}); setTagSelected({}); setTagSearches({});
    setCustomTags({}); setNewTags({});
    setTagLoading(true);
    ...
    try {
      const res = await fetch(`${CATALOGUE_URL}/auto-tag`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_id: product.id, name: product.name, category: product.category || "", description: product.description || "", tier: product.tier || "", occasions: product.occasions || "", price: product.price }),
      });
      if (!res.ok) throw new Error("Auto-tag failed: " + res.status);
      const data = await res.json();
      const grouped = {};
      data.tags.forEach(({ tag, dimension, confidence }) => {
        if (!grouped[dimension]) grouped[dimension] = [];
        grouped[dimension].push({ tag, confidence });
      });
      setTagSuggestions(grouped);
      const initSelected = {};
      DIMENSIONS.forEach(d => { initSelected[d.key] = new Set(); });
      data.tags.filter(t => t.confidence >= 70).forEach(({ tag, dimension }) => {
        if (initSelected[dimension]) initSelected[dimension].add(tag);
      });
      if (existingTags) {
        existingTags.filter(t => t.human_confirmed).forEach(({ tag, dimension }) => {
          if (initSelected[dimension]) initSelected[dimension].add(tag);
        });
      }
  ...
  const saveTags = async () => {
    ...
    try {
      ...
      DIMENSIONS.forEach(({ key }) => {
        ...
        sel.forEach(tag => {
          const suggestion = suggestions.find(s => s.tag === tag);
          rows.push({ product_id: tagProduct.id, tag, dimension: key, confidence: suggestion?.confidence || 100, ai_suggested: !!suggestion, human_confirmed: true });
        });
      });
      if (rows.length) await supabase.from("product_tags").insert(rows);
      const newTagRows = [];
      DIMENSIONS.forEach(({ key }) => {
        const nt = newTags[key] || new Set();
        nt.forEach(tag => newTagRows.push({ tag, dimension: key, created_by: "user" }));
      });
      ...
      setTagProduct(null); loadProducts();
  ...
  const toggleTag = (dimension, tag) => {
    setTagSelected(prev => {
      const next = { ...prev };
      const s = new Set(next[dimension] || []);
      s.has(tag) ? s.delete(tag) : s.add(tag);
      next[dimension] = s; return next;
    });
  };
  ...
  const uploadCSV = async () => {
    ...
    for (const row of csvRows) {
      ...
      if (ins) {
        ...
        fetch(`${CATALOGUE_URL}/auto-tag`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ product_id: ins.id, name: ins.name, category: ins.category || "", description: ins.description || "", tier: ins.tier || "", occasions: ins.occasions || "", price: ins.price }) })
          .then(r => r.json()).then(async (tagData) => {
            if (!tagData.tags?.length) return;
            const tagRows = tagData.tags.map(t => ({ product_id: ins.id, tag: t.tag, dimension: t.dimension, confidence: t.confidence, ai_suggested: true, human_confirmed: false }));
            await supabase.from("product_tags").insert(tagRows);
            await supabase.from("catalog").update({ tagging_status: tagData.tagging_status, tagging_updated_at: new Date().toISOString() }).eq("id", ins.id);
          }).catch(() => {});
        ok++;
      }
    }
              ...
              <>
                <div className="admin-eyebrow">
                  ...
                  <span style={{ fontSize: 11, color: C.muted }}>
                    {products.filter(p=>p.tagging_status==="needs_review").length} need review ·{" "}
                    {products.filter(p=>!p.tagging_status||p.tagging_status==="untagged").length} untagged
                  </span>
                </div>
        ...
        <div className="tag-overlay" onClick={()=>setTagProduct(null)}>
          <div className="tag-panel" onClick={e=>e.stopPropagation()}>
            ...
            <div className="tag-panel-body">
              {tagLoading ? (
                <div className="tag-loading">Analysing product with AI…</div>
              ) : (
                <>
                      ...
                      <div className="tag-dim-card" key={key}>
                        ...
                        <div className="tag-dim-body">
                          {suggestions.map(({tag,confidence}) => {
                            const isSel = selSet.has(tag);
                            return <span key={tag} className={`tag-chip${isSel?" sel":""}`} onClick={()=>toggleTag(key,tag)}>{tag}<span className={`tag-cf${isSel?"":" "+cfClass(confidence)}`}>{confidence}%</span></span>;
                          })}
