  ...
  const results = useMemo(() => {
    ...
    return products.filter(p => {
      ...
      if (params.days && parseInt(params.days) > 0) {
        ...
        const isInStock = fulfillment.state === "in_stock";
        const isLowStock = fulfillment.state === "low_stock";
        if (days <= 2 && !isInStock && !isLowStock) return false;
        if (days <= 5 && !isInStock && !isLowStock) return false;
        if (days <= 7 && !(isInStock || isLowStock) && (lt.includes("5-7") || lt.includes("7") || lt.includes("week") || lt.includes("30") || lt.includes("45"))) return false;
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .p-img{width:100%;padding-bottom:100%;position:relative;overflow:hidden;background:${C.warm};}
      ...
      {tab==="query" && (
        <div className="layout">
          <div className="sidebar">
            ...
            <div style={{marginBottom:20}}>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                ...
                {freeQuery&&<button onClick={clearSearch} style={{background:"none",border:"none",color:"#8A7A72",cursor:"pointer",fontSize:18,padding:"0 4px",lineHeight:1,flexShrink:0}}>&times;</button>}
              </div>
            ...
            <div className="s-field">
              ...
              {tagFilter.exclude_tags.length>0&&(<div className="excl-chips">{tagFilter.exclude_tags.map(t=>(<span key={t} className="excl-chip">{t}<button onClick={()=>removeExcludeTag(t)}>&times;</button></span>))}</div>)}
            ...
            <div style={{marginTop:20,padding:"14px 0",borderTop:"0.5px solid #C8B8B0"}}>
              ...
              <div style={{fontSize:12,color:"#8A7A72"}}>products matched &middot; {selected.size} selected</div>
          ...
          <div className="main">
            {loading?<div className="loading">Loading catalogue&hellip;</div>:(
              <>
                ...
                {results.length===0?<div className="loading">No products match &mdash; try adjusting your filters</div>:(
                ...
                {selected.size>0&&(
                  <div className="sel-bar">
                    <div>
                      ...
                      <div className="sel-qty">{params.qty||1} units &times; {selected.size} products</div>
                    </div>
      ...
      {tab==="admin"&&(
        <div className="admin-layout">
          ...
          <div className="admin-main">
                ...
                <div className="pf-card">
                  ...
                  <div className="pf-card-body">
                    <div className="pf-row pf-row-2">
                      ...
                      <div className="pf-field"><label className="pf-label">Popularity (0&ndash;100)</label><input className="pf-inp" type="number" min="0" max="100" value={form.popularity} onChange={e=>setForm(p=>({...p,popularity:e.target.value}))}/></div>
                    </div>
                  ...
                  <div className="pf-card-body">
                    <div className="pf-row pf-row-1">
                      <div className="pf-field">
                        ...
                        {(form.whats_in_box||[]).length>0&&(<div style={{marginBottom:8,display:"flex",flexDirection:"column",gap:4}}>{form.whats_in_box.map((item,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:C.ink}}><span style={{color:C.muted}}>&mdash;</span><span style={{flex:1}}>{item}</span><button onClick={()=>setForm(p=>({...p,whats_in_box:p.whats_in_box.filter((_,j)=>j!==i)}))} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:14,lineHeight:1,padding:0}}>&times;</button></div>))}</div>)}
                    ...
                    <div className="pf-row pf-row-2">
                      <div className="pf-field"><label className="pf-label">Box dimensions</label><input className="pf-inp" type="text" placeholder="e.g. 20 &times; 15 &times; 10 cm" value={form.box_dimensions} onChange={e=>setForm(p=>({...p,box_dimensions:e.target.value}))}/></div>
                    ...
                    <div className="pf-row pf-row-2">
                      ...
                      <div className="pf-field"><label className="pf-label">In-stock lead time</label><input className="pf-inp" type="text" placeholder="e.g. 7&ndash;10 working days" value={form.lead_time} onChange={e=>setForm(p=>({...p,lead_time:e.target.value}))}/></div>
                    </div>
                  </div>
                </div>
                <div className="pf-card">
                  ...
                  <div className="pf-card-body">
                    <div className="pf-row pf-row-3">
                      ...
                      <div className="pf-field"><label className="pf-label">MTO lead time</label><input className="pf-inp" type="text" placeholder="e.g. 4&ndash;6 weeks" value={form.mto_lead_time} onChange={e=>setForm(p=>({...p,mto_lead_time:e.target.value}))}/><div className="pf-hint">When stock = 0.</div></div>
                    </div>
                    {form.stock_quantity!==''&&(
            ...
            {adminView==="csv"&&(
              <>
                <div className="admin-eyebrow">Bulk Upload &mdash; CSV</div>
                <div style={{maxWidth:640}}>
                  <div style={{background:"#fff",border:"0.5px solid "+C.rule,padding:"20px 24px",marginBottom:16}}>
                    ...
                    <div style={{fontFamily:"'EB Garamond',serif",fontSize:13,color:C.muted,lineHeight:1.7,marginBottom:12}}>
                      ...
                      Optional: category, tier, brand, description, occasions (pipe-separated), image_url, edible, fragile, customisable, popularity, lead_time, moq, box_dimensions, weight_grams, stock_quantity, mto_moq, mto_lead_time, keywords<br/>
                      <span style={{color:C.green}}>Tag columns (optional): intent, audience, persona, sensitivity, perceived_value, usage, functional, brand_signal, style, emotional, occasion_tag, sustainability &mdash; if present, tags are inserted directly without calling the AI.</span><br/>
                      <span style={{color:C.muted}}>Brand defaults to &ldquo;Ikka Dukka&rdquo; if not specified.</span>
                    </div>
                    <a href="/product_upload_template.csv" download style={{fontFamily:"'EB Garamond',serif",fontSize:12,letterSpacing:1.5,textTransform:"uppercase",color:C.cobalt}}>Download template &rarr;</a>
                  </div>
                  ...
                  {csvRows.length>0&&(<div style={{marginBottom:20}}><div style={{fontFamily:"'EB Garamond',serif",fontSize:13,color:C.muted,marginBottom:10}}>{csvRows.length} rows ready &middot; {TAG_DIMS.some(d=>csvRows[0]?.[d])?"Tags detected \u2014 will insert directly":"AI will auto-tag after upload"}</div><button onClick={uploadCSV} disabled={csvUploading} className="f-save">{csvUploading?"Uploading\u2026":"Upload "+csvRows.length+" Products \u2192"}</button></div>)}
          ...
          <div style={{background:C.stone,width:"min(440px,100vw)",display:"flex",flexDirection:"column",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
            <div style={{background:C.sidebar,padding:"20px 28px",flexShrink:0}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                ...
                <button onClick={()=>setDetailProduct(null)} style={{background:"transparent",border:"none",color:"#888",fontSize:22,cursor:"pointer",lineHeight:1,flexShrink:0}}>&times;</button>
              </div>
            </div>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:4}}>
                      {detailProduct.whats_in_box.map((item,i)=>(
                        <div key={i} style={{display:"flex",gap:10,fontSize:13,color:C.ink,alignItems:"flex-start"}}>
                          <span style={{color:C.muted,flexShrink:0}}>&mdash;</span><span>{item}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div style={{background:"#fff",borderTop:"0.5px solid "+C.rule,padding:"16px 28px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>{openTagReview(detailProduct);setDetailProduct(null);}} style={{padding:"9px 16px",border:"0.5px solid "+C.green,background:"transparent",color:C.green,fontSize:10,letterSpacing:1.5,textTransform:"uppercase",cursor:"pointer"}}>Tag &rarr;</button>
          ...
          <div className="tag-panel" onClick={e=>e.stopPropagation()}>
            <div className="tag-panel-head">
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div><div className="tag-panel-name">{tagProduct.name}</div><div className="tag-panel-meta">{tagProduct.category} &middot; {tagProduct.tier} &middot; &#8377;{parseFloat(tagProduct.price).toLocaleString("en-IN")}</div></div>
                <button onClick={()=>setTagProduct(null)} style={{background:"transparent",border:"none",color:"#888",fontSize:20,cursor:"pointer",lineHeight:1}}>&times;</button>
              </div>
            </div>
            <div className="tag-panel-body">
              {tagLoading?<div className="tag-loading">Analysing product with AI&hellip;</div>:(
                <>
                  <div style={{fontSize:13,color:C.muted,marginBottom:14,display:"flex",justifyContent:"space-between"}}><span>Review AI suggestions &middot; tap to toggle &middot; search to add</span><span style={{color:C.ink,fontWeight:500}}>{totalTagSelected} tags</span></div>
            ...
            <div className="tag-panel-foot">
              <div style={{fontSize:13,color:C.muted}}>{newTagCount>0&&<span>{newTagCount} new tag{newTagCount>1?"s":""} added &middot; </span>}{totalTagSelected} tags &middot; {Object.values(tagSelected).filter(s=>s.size>0).length} dimensions</div>
              <div style={{display:"flex",gap:8}}>
    ...
    <div className="prev-overlay">
      <div className="prev-header">
        <div className="prev-title-block">
          <div className="prev-eyebrow">Ikka Dukka &middot; Curated Gift Catalogue</div>
          <div className="prev-title">{params.occasion !== "All" ? params.occasion : "Corporate Gifting"} Collection</div>
          <div style={{fontSize:11,color:"#888",marginTop:4,fontStyle:"italic"}}>Click &times; to remove &middot; scroll down to add more products</div>
        </div>
          ...
          return (
            <div key={p.id} className={"prev-card"+(isActive ? "" : " prev-card-removed")}>
              <div className="prev-img">
                ...
                {isActive ? <button className="prev-remove" onClick={()=>removeProduct(p.id)}>&times;</button> : <button className="prev-restore" onClick={()=>addProduct(p.id)}>+ Add</button>}
              </div>
      ...
      <div style={{maxWidth:1280,margin:"0 auto",width:"100%",padding:"32px 40px 48px",display:"flex",justifyContent:"space-between",alignItems:"flex-end",borderTop:"1.5px solid "+C.ink,marginTop:40}}>
        <div>
          ...
          <div style={{fontSize:11,color:C.muted}}>www.ikkadukka.com &middot; hello@ikkadukka.com</div>
        </div>
        <div style={{textAlign:"right"}}>
          ...
          <div style={{fontSize:11,color:C.muted}}>{qty} unit{qty>1?"s":""} &times; {activeProducts.length} products &middot; excl. GST</div>
        </div>
      </div>
    </div>
  );
